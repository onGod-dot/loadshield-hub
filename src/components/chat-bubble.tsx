...
export function ChatBubble() {
  ...
  useEffect(() => {
    if (open && !minimised) setTimeout(() => inputRef.current?.focus(), 120);
  }, [open, minimised]);

  useEffect(() => {
    if (!open && messages.length > 1) {
      setUnread(messages.filter((m) => m.role === "assistant").length - 1);
    } else {
      setUnread(0);
    }
  }, [open, messages]);
          ...
          >
            ...
            <div className="flex items-center gap-3 border-b border-border bg-primary px-4 py-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-[11px] text-white/70">Powered by Groq</p>
              </div>
